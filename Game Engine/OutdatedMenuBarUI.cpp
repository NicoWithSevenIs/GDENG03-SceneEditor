#include "UI/OutdatedMenuBarUI.h"
...
OutdatedMenuBarUI::OutdatedMenuBarUI(float width, float height)
{
	this->width = width;
	this->height = height;
	this->m_name = "Menu";
...
void OutdatedMenuBarUI::ShowPrompt() {
	if (!doOnPrompt)
		return;
...
void OutdatedMenuBarUI::ShowSaveDialog() {
	if (!show_save_dialog)
		return;
...
void OutdatedMenuBarUI::ShowLoadDialog() {
	if (!show_load_dialog)
		return;
