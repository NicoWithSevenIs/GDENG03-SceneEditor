      ...
      m_originalCout(nullptr)
{
    ...
    // Initialize cout redirection
    EnableCoutRedirection();
    
    // Add initial message
    AddInfo("Debug Console initialized");
}
...
ImVec4 DebugWindow::GetLogColor(const std::string& message) {
    if (message.find("[ERROR]") != std::string::npos) {
        ...
        return ImVec4(0.8f, 0.8f, 0.8f, 1.0f); // Light gray for commands
    }
